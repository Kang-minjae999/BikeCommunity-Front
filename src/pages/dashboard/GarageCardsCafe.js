...
export default function GarageCardsCafe() {
  ...
  const getAllPosts = useCallback(async () => {
    try {
      const response = await axios.get(`/garagepost-service/garagecard/search?page=${page}&size=12&category=카페`);
  ...
  const getAllPosts2 = useCallback(async () => {
    try {
      const response = await axios.get(`/garagepost-service/garagecard/search?page=${page}&size=12&category=커스텀&${params}`);
      if (isMountedRef.current) {
        setPosts(response.data.data.content);
        settotalpage(response.data.data.totalPages);
      }
