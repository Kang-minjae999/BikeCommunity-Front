...
export default function GarageAsks() {
  ...
  const getAllPosts = useCallback(async () => {
    try {
      const response = await axios.get(`/garagepost-service/garageask?page=${page}&size=12`);
  ...
  const getAllPosts2 = useCallback(async () => {
    try {
      const response = await axios.get(`/garagepost-service/garageask/search?page=${page}&size=12&${params}`);
      if (isMountedRef.current) {
        setPosts(response.data.data.content);
        settotalpage(response.data.data.totalPages);
      }
