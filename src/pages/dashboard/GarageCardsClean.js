...
export default function GarageCardsClean() {
  ...
  const getAllPosts = useCallback(async () => {
    try {
      const response = await axios.get(`/garagepost-service/garagecard/search?page=${page}&size=12&category=세차`);
  ...
  const getAllPosts2 = useCallback(async () => {
    try {
      const response = await axios.get(`/garagepost-service/garagecard/search?page=${page}&size=12&category=세차&${params}`);
      if (isMountedRef.current) {
        setPosts(response.data.data.content);
        settotalpage(response.data.data.totalPages);
      }
