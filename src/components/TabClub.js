...
export default function TabClub({TABS, Featured, path, isTab}) {
  ...
  useEffect(() => {
    const index = TABS.map((item) => item.value).findIndex(a => a === tab)
  ...
  return (
    ...
    <Grid item xs={12} lg={8}>
      ...
      />
      <BottomNavigationAction
        sx={{ ...(valueMobile === 3  ? valueStyleRight : valueStyleNone)}}
